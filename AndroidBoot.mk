...
DISABLE_KERNEL_PROTOCOL := DISABLE_KERNEL_PROTOCOL=0
...
ifeq ($(IS_ENABLE_SAIL_FLASHING), 1)
        ENABLE_SAIL_FLASHING := ENABLE_SAIL_FLASHING=1
else
        ENABLE_SAIL_FLASHING := ENABLE_SAIL_FLASHING=0
endif

ifeq ($(IS_ENABLE_SAIL_BOOT), 1)
        ENABLE_SAIL_BOOT := ENABLE_SAIL_BOOT=1
else
        ENABLE_SAIL_BOOT := ENABLE_SAIL_BOOT=0
endif
...
$(TARGET_ABL): $(LOCAL_ABL_SRC_FILE) | $(ABL_OUT) $(INSTALLED_KEYSTOREIMAGE_TARGET)
	$(MAKEPATH)make -C bootable/bootloader/edk2 \
		...
		$(VERIFIED_BOOT_LE) \
		$(EARLY_ETH_ENABLED) \
		$(ENABLE_SAIL_FLASHING) \
		$(ENABLE_SAIL_BOOT) \
		$(EARLY_ETH_AS_DLKM) \
		$(BOOTIMAGE_LOAD_VERIFY_IN_PARALLEL) \
		$(USER_BUILD_VARIANT) \
